          ...
          <div className='grid grid-cols-2 gap-4 mt-3 sm:grid-cols-3'>
            {
              productData.productImg.map((file, idx) => {
                //check if file is already a file(from input) or db object/string
                let preview
                ...
                return (
                  <Card key={idx} className="relative group overflow-hidden">
                    <CardContent>
                      ...
                      className='absolute top-1 right-1 bg-black/50 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition'><X size={14}></X></button>
                    </CardContent>
                  </Card>
                )
      })
